...
class ClientAPI {
  ...
  #get_user_agent() {
    ...
    console.log(`[Akun ${this.accountIndex + 1}] Create user agent..`.blue);
  ...
  checkInStatus(checkInArray) {
    ...
    if (status === "0") {
      ...
      result = "Today has been skipped.";
    } else if (status === "2") {
      result = "Not checked in today.";
...
async function main() {
  ...
  if (!resCheck.endpoint) return console.log(`API ID could not be found, connection error possible, try again later!`.red);
        ...
        workerPromises.push(
          new Promise((resolve) => {
            ...
            worker.on("exit", (code) => {
              worker.terminate();
              if (code !== 0) {
                errors.push(`Workor for account ${currentIndex} exit with code: ${code}`);
              }
              resolve();
            });
          })
